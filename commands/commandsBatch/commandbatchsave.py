...
class CommandBatchSave(Command):
    ...
    def __write_file(self, batch):
        with open("files/" + self.__file_name + ".rawbatch", 'w') as batch_file:
            return batch_file.write(batch.get_content())

    def execute(self):
        batch = CommandBatchSave.data_batch.get_names().get(self.__batch_name)
        if batch is None:
